...
if user_query:
    ...
    if st.session_state.pipeline.get("report_md"):
        # 보고서가 이미 있으면 -> 추가 질문으로 간주
        with st.spinner("답변을 생성 중입니다..."):
            reply = handle_followup(user_query)
        st.session_state.messages.append(AIMessage(content=reply))
        st.rerun() # 새 답변을 포함하여 화면을 다시 그림
    else:
        # 보고서가 없으면 -> 새로운 분석으로 간주
        st.session_state.log_entries = [] # 새 분석이므로 로그 초기화
        run_full_pipeline_resumable(user_text=user_query)


# 2. 로그 및 보고서 UI 렌더링 (분석 중이 아닐 때)
#    분석 중일 때는 아래 3번 블록에서 처리
if not st.session_state.pipeline.get("running"):
    ...
    if st.session_state.pipeline.get("report_md"):
        ...
        st.markdown(st.session_state.pipeline["report_md"], unsafe_allow_html=True)


# 3. 파이프라인 실행 및 UI 렌더링 (분석 중일 때)
if st.session_state.pipeline.get("running"):
    # ✅ 스피너를 이 곳에서 사용하여 전체 분석 과정을 감쌉니다.
    with st.spinner("⏳ AI가 데이터를 분석하고 있습니다..."):
        st.write("---")
        st.subheader("🤖 AI 에이전트 분석 과정")
        log_container = st.container(border=True)
        logger = StreamlitLogHandler()
        logger.render(log_container) # 현재까지의 로그를 먼저 그림
