...
보고서는 solution_direction에 부합하는 구체적인 전략과 실행 계획을 포함해야 합니다.
데이터와 직접 연결된 구체적인 실행 방안을 제시하세요. 외부 환경 데이터에서 도출된 인사이트도 전략에 반영해야 합니다.

**보고서 구조 지침 (반드시 준수):**

1.  **서론:** 문제 배경 및 분석 목적을 간결하게 설명합니다.
2.  **본론:**
    * **2.1. 데이터 기반 현황 분석:**
        * 초기 6개월 평균과 최근 6개월 평균 성과를 비교 분석할 때는 **반드시 Markdown 테이블 형식**을 사용하여 명확하게 제시합니다. 테이블에는 '지표', '초기 6개월 평균', '최근 6개월 평균', '변화율', '인사이트' 컬럼을 포함하세요.
        * 최근 3개월 동향과 외부 환경 분석 결과를 요약합니다.
    * **2.2. 핵심 문제점 요약:** 분석 결과를 바탕으로 가장 시급하고 중요한 문제점을 명확하게 정의합니다.
    * **2.3. 목표 (KPIs):** 문제 해결을 위해 측정 가능한 핵심 성과 지표(KPI)들을 **별도의 섹션으로 명확하게 나열**합니다. (컨텍스트의 'kpis' 목록 활용)
    * **2.4. 전략 제안 및 실행 계획:**
        * 제시된 각 전략은 **반드시 다음 하위 구조**를 따라야 합니다:
            * **목표:** 해당 전략을 통해 달성하고자 하는 구체적인 목표
            * **근거:** 왜 이 전략이 필요한지에 대한 데이터 기반의 명확한 논리 (컨텍스트의 데이터 활용)
            * **실행 계획:** 목표 달성을 위한 구체적이고 실행 가능한 세부 방안들 (번호 매기기 또는 불릿 포인트 사용)
3.  **결론:** 제안된 전략 실행 시 기대되는 효과를 요약하고 마무리합니다.
...
def ensure_state():
    ss = st.session_state
    if "messages" not in ss: ss.messages = [SystemMessage(content=system_prompt), AIMessage(content=greeting)]
...
def create_kpi_charts(kpi_list: list, timeseries_df: Optional[pd.DataFrame]) -> list:
    ...
    if timeseries_df is None or timeseries_df.empty:
        ...
        return charts # 데이터 없으면 빈 리스트 반환

    # ✅ [수정] 매칭 로직 개선: AI KPI 설명에 컬럼명이 포함되어 있는지 확인
    valid_kpi_columns = []
    for col in PLOTTING_NUMERIC_COLS: # 그래프 가능 컬럼을 기준으로 순회
        if col in timeseries_df.columns: # 실제 데이터에 컬럼이 존재하는지 확인
            for kpi_item in kpi_list: # AI가 생성한 KPI 목록 순회
                if col in str(kpi_item): # KPI 설명 문자열 안에 컬럼명이 포함되면
                    valid_kpi_columns.append(col)
                    break # 해당 컬럼은 찾았으므로 다음 컬럼으로 넘어감

    # 디버깅용 로그 추가
    console_logger.info(f"📈 AI KPI 목록: {kpi_list}")
    console_logger.info(f"📈 그래프 대상 컬럼 필터링 결과: {valid_kpi_columns}")

    # 중복을 제거하고 최대 4개의 그래프만 그림
    plotted_cols_count = 0
    for kpi_col in sorted(set(valid_kpi_columns)):
        if plotted_cols_count >= 4: break # 최대 4개 제한
        try:
            fig = px.line(
                timeseries_df,
                x='기준년월',
...
if st.session_state.awaiting_candidate and st.session_state.candidates:
    ...
    if st.button("선택 확정", type="primary"):
        ...
        # ✅ merchant_id 대신 target_store_info가 설정되었으므로 바로 analysis 단계로
        _set_step("analysis")
        st.rerun()
    st.stop()
