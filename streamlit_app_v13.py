...
**RAG 참고 자료 활용법:**
- '4_업종별_RAG_참고_자료'가 제공된 경우, 이 정보를 **창의적인 아이디어나 업종 특화 전략의 근거**로 활용하세요.
- RAG 자료의 내용을 단순 복사-붙여넣기 하지 마세요.
- RAG 자료의 내용을 해당 '업종'에 특화된 마케팅 전략(예: 프로모션, 신메뉴 제안, 타겟 고객층 공략법)을 제안할 때 **참고용 근거**로 사용하고, (출처: RAG 참고 문서 X)와 같이 표기할 수 있습니다.
...
"""
    ...
    def __init__(self, sync_model: Any, csv_path: str, json_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            console_logger.info(f"RAG: FAISS 벡터 저장소 로드 완료: {db_path}")

        except ImportError:
             console_logger.error("❌ RAG: 'langchain_community' 관련 라이브러리가 설치되지 않았습니다.")
             # self.vectorstore는 None으로 유지됨
        except Exception as e:
            console_logger.error(f"❌ RAG: 벡터 저장소 '{db_path}' 로드 중 오류 발생: {e}")
            # self.vectorstore는 None으로 유지됨
    ...
    def _retrieve_rag_context(self, industry: Optional[str]):
        """
        RAG 모듈: 확정된 업종 명을 기반으로 FAISS 벡터 저장소에서 마케팅 문서를 검색합니다.
        """
        console_logger.info(f"RAG: '{industry}' 업종에 대한 RAG 검색 시작...")
        
        if not self.vectorstore:
            console_logger.warning("RAG: 벡터 저장소가 로드되지 않아 검색을 건너뜁니다.")
            self.rag_context = "RAG 검색 실패: 벡터 저장소가 초기화되지 않았습니다."
            self.context['rag_context'] = self.rag_context
            return

        if not industry:
            console_logger.warning("RAG: 업종 정보가 없어 검색을 건너뜁니다.")
            self.rag_context = "RAG 검색 건너뜀: 업종 정보가 없습니다."
            self.context['rag_context'] = self.rag_context
            return
        ...
        try:
            ...
            if not context_docs:
                console_logger.warning(f"RAG: '{industry}'에 대한 문서를 찾지 못했습니다.")
                self.rag_context = f"RAG 검색: '{industry}' 관련 참고 문서를 찾지 못했습니다."
                self.context['rag_context'] = self.rag_context
                return
            ...
            self.rag_context = "\n\n".join(context_parts)
            self.context['rag_context'] = self.rag_context # 에이전트 컨텍스트에 저장
            console_logger.info(f"RAG: '{industry}' 업종에 대해 {len(context_docs)}개 문서 검색 완료.")

        except Exception as e:
            console_logger.error(f"❌ RAG: '{industry}' 검색 중 오류 발생: {e}")
            self.rag_context = f"RAG 검색 중 오류 발생: {e}"
            self.context['rag_context'] = self.rag_context
    ...
    def _generate_strategy_and_report(self) -> str:
        ...
        rag_res = self.context.get('rag_context')
        ...
        final_context = {
            ...
            "4_업종별_RAG_참고_자료": rag_res if rag_res else "제공된 RAG 참고 자료 없음"
        }

        prompt_template = self._GENERATE_STRATEGY_REPORT_SYSTEM_PROMPT.format(solution_direction=solution_direction)
...
def run_full_pipeline_resumable(user_text: str | None = None):
    ...
    if P["step"] == "analysis":
        ...
        if not P["pd_out"] or "error" in P["pd_out"]: 
        ...
        # --- RAG 모듈 호출 ---
        logger.log("🔍 3-3: 업종 특화 마케팅 정보 (RAG) 검색 중...")

        # '업종'를 우선 사용하고, 없으면 '업종_분류'을 사용
        industry_name = P["exploration"].get("meta", {}).get("업종")
        if not industry_name:
            industry_name = P["exploration"].get("meta", {}).get("업종_분류")
        
        agent._retrieve_rag_context(industry_name) # RAG 메서드 호출
        P["rag_context"] = agent.context.get("rag_context") # 결과를 세션에 저장

        # RAG 결과에 따라 로그 분기
        if P["rag_context"] and "실패" not in P["rag_context"] and "없음" not in P["rag_context"] and "찾지 못했습니다" not in P["rag_context"]:
            logger.log("✅ 3-4: 업종 특화 정보 검색 완료", details={"retrieved_context_summary": f"{len(P['rag_context'])} 자 분량의 컨텍스트 확보됨"}, expander_label="검색된 RAG 문서 보기")
        else:
             logger.log("⚠️ 3-4: 업종 특화 정보(RAG)를 찾지 못했거나 건너뛰었습니다.", details={"reason": P["rag_context"]})
        ...
        logger.log("✔️ 3-5: 맞춤형 마케팅 전략 생성 완료")
